...
28. `scripts/workflow/sync_mac_from_main.sh` and `scripts/workflow/check_release_ready.sh` remain only as legacy transition tools while old `mac` work is being drained. They are not part of the normal direct-to-`main` workflow.
