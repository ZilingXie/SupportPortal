...
14. The finalization flow for a task branch is: confirm the task worktree is on the expected named branch and not detached HEAD; ensure task changes are committed and non-task artifacts are excluded; run fresh verification on `codex/<task>`; move to a clean `mac` worktree; sync `main` into `mac`; merge the task branch into `mac`; confirm the merged result is safely present on `mac`; then delete the local task branch.
15. Every time `mac` is promoted to `main`, first sync `main` into `mac` again, then run release-ready checks from a clean `mac` worktree. Open a new `mac -> main` PR with `base=main` and `head=mac`; do not reuse an older PR or stale compare view. Use `scripts/workflow/check_release_ready.sh` or equivalent manual checks.
