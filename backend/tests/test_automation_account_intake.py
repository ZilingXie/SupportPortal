...
class _FakeRepository:
    def __init__(self) -> None:
        ...
        self.relay_requests: dict[str, dict] = {}
    ...
    def create_enablement_relay_request(self, **kwargs):
        request_id = str(kwargs.get("request_id") or "")
        if not request_id or request_id in self.relay_requests:
            return None
        request = dict(kwargs)
        request.update(
            {
                ...
                "batch_id": None,
                "suppression_reason": "",
                "idempotency_key": f"enablement-relay:{request_id}",
            }
        )
        self.relay_requests[request_id] = request
        return dict(request)

    def get_enablement_relay_request(self, request_id):
        request = self.relay_requests.get(str(request_id or ""))
        return dict(request) if request else None

    def finish_enablement_relay_request(self, *, request_id, status, now, reason=""):
        request = self.relay_requests.get(str(request_id or ""))
        if request is None:
            return None
        request = dict(request)
        request["status"] = status
        request["suppression_reason"] = reason
        request["updated_at"] = now
        self.relay_requests[str(request_id)] = request
        return dict(request)
...
class AutomationAccountIntakeTest(unittest.TestCase):
    ...
    def _run_auto_enablement(self, repository, **patches):
        with patch.dict(os.environ, {"ENABLEMENT_WORKFLOW_MODE": "archer"}):
            return self._run_complete_enablement(repository, **patches)

    def test_enablement_auto_mode_gates_confirmation_and_creates_request(self):
        repository = _FakeRepository()
        sent = []
        outcome = self._run_auto_enablement(
            repository,
            send_enablement_internal_email=lambda payload: sent.append(payload),
        )
        # The confirmation reply job is created first and carries the auto
        # copy contract; the application is gated behind the public readback
        # and a relay request row is persisted for exactly one dispatch.
        self.assertIsNotNone(outcome["reply_job"])
        self.assertEqual(outcome["reply_job"]["reply_intent"], "submission_confirmation")
        self.assertEqual(outcome["reply_job"]["reply_facts"]["activation_sla"], "business-day review")
        self.assertFalse(outcome["reply_job"]["close_after_publish"])
        self.assertEqual(outcome["internal_email_send_status"], "awaiting_public_reply")
        self.assertEqual(outcome["account_case"]["internal_email_send_reason"], "enablement_auto_review")
        self.assertEqual(sent, [])
        self.assertIsNone(outcome["account_case"]["internal_email_payload"])
        request = repository.relay_requests["enr-AC-123-v1"]
        self.assertEqual(request["status"], "gated")
        self.assertEqual(request["app_id"], "0123456789abcdef0123456789abcdef")
        self.assertEqual(request["workflow_mode"], "archer")
        self.assertEqual(request["reply_job_id"], outcome["reply_job"]["job_id"])
        self.assertEqual(request["target_params"]["maxSubscribeLoad"], 10)
        self.assertEqual(request["target_params"]["region"], 2)
        workflow = outcome["account_case"]["automation_context"]["enablement_auto_workflow"]
        self.assertEqual(workflow["state"], "awaiting_public_reply")
        self.assertEqual(workflow["request_id"], "enr-AC-123-v1")
        events = [event for _, event, _ in repository.events]
        self.assertIn("enablement_auto_dispatch", events)

    def test_enablement_auto_invalid_appid_asks_again_without_request(self):
        repository = _FakeRepository()
        outcome = self._run_auto_enablement(
            repository,
            app_id="frhug123",
            send_enablement_internal_email=lambda payload: None,
        )
        self.assertEqual(outcome["reply_job"]["reply_intent"], "enablement_appid_invalid")
        self.assertEqual(outcome["account_case"]["missing_fields"], ["app_id"])
        self.assertEqual(outcome["internal_email_send_status"], "not_applicable")
        self.assertEqual(repository.relay_requests, {})

    def test_enablement_auto_superseded_appid_bumps_request_version(self):
        repository = _FakeRepository()
        # A gated auto application whose customer corrects the App ID before
        # dispatch: the resume path reloads the case with its prior workflow,
        # so the old request is superseded and version 2 is created.
        prior_case = {
            ...
            "collected_fields": {
                "app_id": "ffffffffffffffffffffffffffffffff",
                "requested_feature": "media_relay",
            },
            "automation_context": {
                "enablement_auto_workflow": {
                    ...
                    "request_id": "enr-AC-123-v1",
                    "request_version": 1,
                    "app_id": "0123456789abcdef0123456789abcdef",
                }
            },
        }
        repository.relay_requests["enr-AC-123-v1"] = {
            "request_id": "enr-AC-123-v1",
            "account_case_id": "AC-123",
            "app_id": "0123456789abcdef0123456789abcdef",
            "status": "gated",
        }
        with patch.dict(os.environ, {"ENABLEMENT_WORKFLOW_MODE": "archer"}), self._base_patches():
            updated, _reply_job, outcome = intake_module._start_enablement_auto_dispatch(
                repository=repository,
                account_case=dict(prior_case),
                ticket_id="123",
                customer_email="c@example.com",
                persona_assignment=None,
                processing_profile="production",
                trigger_message_created_at="2026-09-16T00:00:00+00:00",
            )
        self.assertEqual(outcome, "review_requested")
        workflow = updated["automation_context"]["enablement_auto_workflow"]
        self.assertEqual(workflow["request_version"], 2)
        self.assertEqual(repository.relay_requests["enr-AC-123-v1"]["status"], "cancelled")
        self.assertEqual(repository.relay_requests["enr-AC-123-v2"]["app_id"], "ffffffffffffffffffffffffffffffff")

    def test_enablement_default_mode_creates_no_relay_request(self):
        repository = _FakeRepository()
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ENABLEMENT_WORKFLOW_MODE", None)
            outcome = self._run_complete_enablement(repository)
        # Manual default: no relay request, manual gate keys, 24h copy intact.
        self.assertEqual(repository.relay_requests, {})
        self.assertEqual(outcome["internal_email_send_status"], "awaiting_public_reply")
        self.assertEqual(outcome["account_case"]["internal_email_send_reason"], "enablement_manual_review")
        workflow = outcome["account_case"]["automation_context"]["enablement_manual_workflow"]
        self.assertEqual(workflow["state"], "awaiting_public_reply")
