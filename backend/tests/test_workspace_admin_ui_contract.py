...
class WorkspaceAdminUiContractTests(unittest.TestCase):
    ...
    def test_account_automation_hierarchical_agent_config_and_environment_tabs_are_operational(self) -> None:
        ...
        self.run_admin_app_script(
            ...
            automationData.cases[0].token_usage = { available: true, error_reason: null, total_input_tokens: 1234, total_cached_input_tokens: 210, total_output_tokens: 567, total_embedding_tokens: 89, cost_usd: { available: false, total_usd: null, by_model: [{ provider: 'openai', model: 'gpt-test', usd: null }] }, token_by_model: [{ provider: 'openai', model: 'gpt-test', input_tokens: 1234, cached_input_tokens: 210, output_tokens: 567, embedding_tokens: 89 }], sources: { rag: { available: true, total_input_tokens: 900, total_cached_input_tokens: 150, total_output_tokens: 400, total_embedding_tokens: 89, stage_totals: { rag_answer: { input_tokens: 900, output_tokens: 400, calls: 1 } } }, automation: { available: true, call_count: 2, total_input_tokens: 334, total_cached_input_tokens: 60, total_output_tokens: 167, stage_totals: { quota_field_extractor: { input_tokens: 334, output_tokens: 167, calls: 2 } } } } };
            automationData.token_usage_page_total = { total_input_tokens: 1234, total_cached_input_tokens: 210, total_output_tokens: 567, total_embedding_tokens: 89, cost_usd_total: 0, cost_usd_available: false };
            ...
            if (!tokenMarkup.includes('210 cached') || !tokenMarkup.includes('admin-token-cached')) throw new Error('cached token cell summary missing');
            if (!tokenMarkup.includes('data-action="toggle-token-detail"')) throw new Error('token usage toggle missing');
            if (!tokenMarkup.includes('admin-token-cost-unknown') || !tokenMarkup.includes('$—')) throw new Error('unpriced cost badge missing');
            ...
            if (!tokenDetailMarkup.includes('<th>Cached</th>')) throw new Error('by-model cached column missing');
            if (!tokenDetailMarkup.includes('<th>Cost</th>') || !tokenDetailMarkup.includes('model pricing not configured')) throw new Error('model cost column missing');
            automationData.cases[0].token_usage.cost_usd = { available: true, total_usd: 0.0123, by_model: [{ provider: 'openai', model: 'gpt-test', usd: 0.0123 }] };
            automationData.token_usage_page_total.cost_usd_available = true;
        ...
        self.assertIn("20260825-cached-display-1", index)
        for marker in (
    ...
    def test_workspace_admin_login_uses_transactional_entry_contract(self) -> None:
        ...
        self.assertIn("20260825-cached-display-1", html)
