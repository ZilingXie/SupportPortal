...
class EngineerSlackWorkflowContractTests(unittest.TestCase):
    ...
    def test_runbook_assigns_outbound_to_supportportal(self) -> None:
        ...
        self.assertIn("currently lacks cryptographic request verification", raw)
        self.assertIn("cannot provide environment variables", raw)
        self.assertIn("exact raw request body", raw)
        self.assertNotIn("N8N_BLOCK_ENV_ACCESS_IN_NODE=false", raw)
        self.assertNotIn("/api/integrations/slack/verify-request", raw)
