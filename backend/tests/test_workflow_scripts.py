...
class WorkflowScriptTests(unittest.TestCase):
    ...
    def test_restart_single_host_stack_prints_build_cache_diagnostics_and_honors_no_cache_flag(self) -> None:
        ...
        self._write(seed, "requirements.base.lock", "fastapi==0.1\n")
        self._write(seed, "requirements.full.lock", "fastapi==0.1\ntorch==0.1\n")
        self._write(seed, "deployment/docker-compose.single-host.yml", "services: {}\n")
        self._write(seed, "deployment/docker-compose.single-host.local-lightweight.yml", "services: {}\n")
        ...
        expected_base_lock_hash = _git(["hash-object", "requirements.base.lock"], cwd=repo).stdout.strip()
        expected_full_lock_hash = _git(["hash-object", "requirements.full.lock"], cwd=repo).stdout.strip()
        ...
        self.assertIn(f"requirements.base.lock: {expected_base_lock_hash}", result.stdout)
        self.assertIn(f"requirements.full.lock: {expected_full_lock_hash}", result.stdout)
