...
def build_router_system_prompt(*, route_examples: list[dict[str, Any]]) -> str:
    ...
    return "\n".join(
        [
            ...
            "You must output automation_eligibility for billing intents:",
            "- eligible: only for billing.detailed_invoice with complete transaction/date/amount data and no dispute/refund signals",
            "- not_eligible: for billing.account_suspension (needs human review), billing.refund_or_dispute (always needs human), or billing.general",
            "",
            "## Decision Rules",
            ...
            "For non-billing scope_labels, semantic_intent, automation_eligibility, evidence_spans, and risk_flags should be null or empty.",
            "",
            "## Few-shot Examples",
            "\n\n".join(examples),
        ]
