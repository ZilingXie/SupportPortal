...
def _assert_no_positive_suspension_close_claim(reply: str) -> None:
    ...
    Negations such as "we will not close this ticket" stay allowed.
    """
    close_claim = re.compile(r"(?i)\b(?:clos\w*|archiv\w*|reop\w*)\b")
    if any(close_claim.search(clause) and _is_positive_clause(clause) for clause in _reply_clauses(reply)):
