...
Claude Code follows the repository authority and workflow in `AGENTS.md`. `REASONIX.md` remains the Reasonix handoff-before-review exception.
...
4. `main` is PR-only and squash-only. After targeted verification, use `scripts/workflow/finalize_task_to_main.sh`; it owns refresh, push, PR, merge, root fast-forward, CodeGraph sync, and current-task cleanup. A merged PR is an intermediate finalization state, not task completion: report a task complete only after that script succeeds and confirms removal of its workspace and local `codex/*` branch. If merge succeeds but cleanup does not, report `merged, cleanup pending` with the branch, workspace, and blocker, then retain ownership until cleanup finishes. Never create or use `mac` or `mac-integration` workflows.
