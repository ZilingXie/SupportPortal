...
Claude Code follows the same repository authority and workflow as Codex. This file intentionally mirrors the hot-path rules in `AGENTS.md` so Claude Code may review, modify code/docs, verify, commit, create/reuse PRs, finalize, and clean up when assigned.
...
## Machine Runtime
1. Use ordinary shell commands directly on this machine.
2. This machine runs SupportPortal with Docker and Docker Compose.
3. The deployment entrypoint is `./deploy_ec2.sh`, a root wrapper around `deployment/deploy_ec2.sh`.
4. When asked to pull the latest changes and deploy, run `./deploy_ec2.sh --branch main` from the root workspace. The script handles fetch/pull, Docker build, service restart, and internal/external `/health` checks.

## Non-Negotiables
1. Do not add fixed preflight steps. Read AgentMemory, skill files, or Git/worktree state only when this task needs that context.
...
3. Use `rg` for docs, literal text, comments, config keys, logs, rules, and other non-structural searches. For code understanding, use the narrowest repo-native search or file reads needed for the task.
...
7. `main` is PR-only and squash-only. Finalization pushes the task branch, opens/reuses a PR to `main`, squash-merges, fast-forwards root `main`, and cleans only the current task workspace/branch.
...
5. Stack-relevant tasks touch `ui/`, `backend/`, `deployment/`, compose/restart scripts, runtime config, or official Docker runtime behavior. They require post-merge live verification from root `main`.
6. Stack verification on this machine uses Docker deployment: run `./deploy_ec2.sh --branch main`, verify `/health`, and check one task-specific live marker.
...
5. Squash-merge, wait for GitHub to report merged, fast-forward root `main`, and delete only the current task workspace/local branch.
...
- You need the trigger matrix for AgentMemory, skills, or Git/worktree state.
