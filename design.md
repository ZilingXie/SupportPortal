...
14. `/workspace/admin` 登录后的 rail 在桌面默认使用 `96px` icon-only 状态，hover 或 focus-within 时展开到 `264px`；rail footer 只保留默认可见的 Logout 图标。每个 tab 共用主内容列内的顶部栏，当前 tab title 位于左侧，当前账号头像、名称与角色位于右侧；顶部栏不得跨入或覆盖 rail。
...
18. Admin 响应式 rail 规则固定为：`>= 901px` 支持 hover/focus 展开，`721px - 900px` 保持 icon-only，`<= 720px` 退化为顶部静态导航并显示必要标签与 Logout；工作区顶部栏在窄屏仍须保持 tab title 与账号信息同一行，通过稳定宽度、单行省略和间距约束避免重叠或页面级横向溢出。
...
15. Workspace Admin（`/workspace/admin/`）是唯一的 Admin 控制台，作为 ECS Production 的只读运营视图：全部业务读端点（accounts、cases、metrics、audit、engineer schedules、Automated Cases、Agent Config）必须从 ECS Production schema 经只读 reader 读取，不得读 EC2 本地业务库；登录与账号体系保留 EC2 本地。ECS 环境侧不提供任何 Admin 页面（`/automation/<environment>/admin/` 不存在）。所有业务写操作（dispatch、reassign-due、invitation、engineer schedule、case assignment、Prompt 草稿/调度/恢复、Persona 编辑）前后端一律禁用——后端写端点 fail closed，前端使用内建的只读控件逻辑；Prompt 与 Persona 对 ECS Production 只经发布通道变更。`Environment Config` 维持读取 EC2 .env 的配置名 inventory（非业务库数据）。
16. `Release Notes` 是 Workspace Admin 的 section，位于 `Automated Cases` 正下方，含两个视图：`Versions` 人读视图按语义化版本（major.minor.patch）倒序展示，数据来自仓库 `docs/release_notes.json`，由发布 agent 按仓库规则维护（基线 1.0.0=当前 Production；新增能力→minor，修复/配置/开关/下架→patch，整体批次晋升或破坏性变更→major）；`Deployment records` 机器视图实时读取 ECS Production 的部署落库记录（release、commit、镜像 digest、PR 变更列表、部署时间）。两个视图都必须使用 `no-store`。
...
7. Internal Automation Email 使用稳定的 Light 内容画布：Outlook 外层界面可以处于深色主题，但邮件正文、主卡片和信息区必须保持清晰的浅色 surface，不得依赖客户端自动反色或输出第二套暗色 palette。
...
7. 侧栏 `Rerun`（全量）与 detail header 的 `Rerun this execution`、侧栏 `Reset environment` 属于危险操作；确认弹窗必须冻结目标（全量 rerun 冻结目标 execution 数量，单条 rerun 冻结 case_id/有 ticket 时冻结 ticket 号）；preproduction 的 rerun 弹窗必须说明 eligible 路由会向 Zendesk 工单写入 internal comment。全量 rerun 逐个 execution 顺序执行并在侧栏显示进度面板（processed/total、succeeded/failed、失败 case 明细，`aria-live="polite"`）；任何 rerun/reset 运行期间所有 rerun/reset 入口禁用。
