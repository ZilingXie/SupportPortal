...
6. `/client2` correspondence detail 的 header meta 允许在 `Updated` 右侧追加 reply ETA chip，但只允许在最新一条可见消息来自客户时显示；agent 最新发言时必须隐藏。展示文案规则固定为：`< 60 min` 显示 `Next update in MM:00`，`>= 60 min` 显示 `Next update in Hh MMm`。
