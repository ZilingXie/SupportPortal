...
def validate_entity_types(entity_types):
    ...
    for name in entity_types:
        if not isinstance(name, str) or not name.isidentifier():
            raise ValueError(f'Invalid entity type name: {name}')
...
class Graphiti:
    def __init__(
        ...
        self.cross_encoder = cross_encoder  # None by default (use RRF reranker)
    ...
    async def _extract_and_resolve_nodes(
        ...
        extracted_nodes, node_episode_index_map = await extract_nodes(
            ...
            second_pass_extraction=second_pass_extraction,
            second_pass_mode=second_pass_mode,
            second_pass_min_entities=second_pass_min_entities,
        )
    ...
    async def _extract_and_resolve_edges(
        ...
        extracted_edges = await extract_edges(
            ...
            second_pass_extraction=second_pass_extraction,
            second_pass_mode=second_pass_mode,
            second_pass_min_edges=second_pass_min_edges,
        )
    ...
    async def add_episode(
        ...
        second_pass_extraction : bool
            Optional. Whether to run a second LLM pass over first-pass nodes and edges before
            graph resolution and persistence. Defaults to False for API compatibility.
        second_pass_mode : str
            Optional. "always" runs the second pass whenever enabled; "conditional" only runs
            it when first-pass extraction falls below configured thresholds.
        saga : str | SagaNode | None
        ...
        with self.tracer.start_span('add_episode') as span:
            try:
                ...
                # Schema hard validation (KAG-lite)
                if entity_types or edge_types:
                    from graphiti_core.schema_validate import validate_against_schema

                    hydrated_nodes, entity_edges = validate_against_schema(
                        hydrated_nodes,
                        entity_edges,
                        entity_types,
                        edge_types,
                ...
                if update_communities:
                    update_results = await semaphore_gather(
                        *[
                    ...
                    communities = [c for result in update_results for c in result[0]]
                    community_edges = [e for result in update_results for e in result[1]]
    ...
    async def build_communities(
        ...
        # Save old community UUIDs BEFORE building (only delete OLD ones later)
        # Only track communities in the same group_ids being rebuilt
        if group_ids:
            old_records, _, _ = await driver.execute_query(
                'MATCH (c:Community) WHERE c.group_id IN $group_ids RETURN c.uuid AS uuid',
                group_ids=group_ids,
            )
        else:
            old_records, _, _ = await driver.execute_query(
                'MATCH (c:Community) RETURN c.uuid AS uuid'
            )
        old_uuids = {r['uuid'] for r in old_records}

        # Build + persist new communities
        ...
        # Only delete OLD communities (not the newly saved ones)
        if old_uuids:
            await driver.execute_query(
                'MATCH (c:Community) WHERE c.uuid IN $uuids DETACH DELETE c',
                uuids=list(old_uuids),
            )
