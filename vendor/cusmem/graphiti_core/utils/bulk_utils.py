...
async def add_nodes_and_edges_bulk_tx(
    ...
    for node in entity_nodes:
        ...
        if driver.provider == GraphProvider.KUZU:
            ...
            for k, v in neo4j_safe_attributes(node.attributes or {}).items():
    ...
    for edge in entity_edges:
        ...
        if driver.provider == GraphProvider.KUZU:
            ...
            from graphiti_core.helpers import neo4j_safe_attributes
            for k, v in neo4j_safe_attributes(edge.attributes or {}).items():
...
async def dedupe_nodes_bulk(
    ...
    for _, resolved_nodes in episode_resolutions:
        for node in resolved_nodes:
            ...
            node_key = _dedup_key(node.name, node.labels)
            exact_match = next(
                (
                    candidate
                    for candidate in existing_candidates
                    if _dedup_key(candidate.name, candidate.labels) == node_key
                ),
                None,
            )
