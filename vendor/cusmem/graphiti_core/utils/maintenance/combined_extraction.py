...
from graphiti_core.utils.maintenance.dedup_helpers import _dedup_key, _normalize_string_exact
...
async def extract_nodes_and_edges(
    ...
    # Use type-aware key; when multiple nodes share same name+type, last one wins.
    name_to_node: dict[tuple, EntityNode] = {
        _dedup_key(node.name, node.labels): node for node in extracted_nodes
    }

    extracted_edges: list[EntityEdge] = []
    for edge_data in response_object.edges:
        # Look up source and target by name (try type-aware key first, then name-only fallback)
        src_name = _normalize_string_exact(edge_data.source_entity_name)
        tgt_name = _normalize_string_exact(edge_data.target_entity_name)
        # Try to find by any type key matching this name
        source_node = next(
            (n for k, n in name_to_node.items() if k[0] == src_name), None
        )
        target_node = next(
            (n for k, n in name_to_node.items() if k[0] == tgt_name), None
        )
