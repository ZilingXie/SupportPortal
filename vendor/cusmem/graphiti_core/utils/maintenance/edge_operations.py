...
def _add_name_to_node_map(
    name_to_node: dict[str, EntityNode | None],
    name: str,
    node: EntityNode,
) -> None:
    cleaned = name.strip()
    ...
    existing = name_to_node.get(cleaned)
    if existing is None and cleaned in name_to_node:
        return
    if existing is not None and existing.uuid != node.uuid:
        name_to_node[cleaned] = None
        return
    name_to_node[cleaned] = node


def _build_name_to_node_map(nodes: list[EntityNode]) -> dict[str, EntityNode | None]:
    name_to_node: dict[str, EntityNode | None] = {}
    for node in nodes:
        _add_name_to_node_map(name_to_node, node.name, node)
        official_name = node.attributes.get('official_name') if node.attributes else None
        if official_name:
            _add_name_to_node_map(name_to_node, str(official_name), node)

        synonyms = node.attributes.get('synonyms') if node.attributes else None
        if synonyms:
            synonym_values = synonyms if isinstance(synonyms, list) else [synonyms]
            for synonym in synonym_values:
                _add_name_to_node_map(name_to_node, str(synonym), node)
    return name_to_node


def _classify_entity_miss(
    name: str,
    name_to_node: dict[str, EntityNode | None],
) -> str:
    original = name.strip()

    for node_key in name_to_node:
        if not node_key:
            continue
        if len(original) < 3 or len(node_key) < 3:
            continue
        if original in node_key or node_key in original:
            return 'fuzzy_near_miss'
    return 'ghost'
...
def _resolve_node_name(
    ...
    return name_to_node.get(name.strip())


class _ValidationResult:
    """Result of validating extracted edges against the entity name map."""

    __slots__ = ('valid_edges', 'connected_names', 'dropped_count', 'rejected_edges')
    ...
    def __init__(
        ...
        self.connected_names = connected_names
        self.dropped_count = dropped_count
        self.rejected_edges = rejected_edges or []


def _candidate_name_for_miss(
    name: str,
    name_to_node: dict[str, EntityNode | None],
) -> str | None:
    original = name.strip()

    node = name_to_node.get(original)
    if node is not None:
        return node.name

    if len(original) < 3:
        return None
    for node_key, node in name_to_node.items():
        if node is None or not node_key or len(node_key) < 3:
            continue
        if original in node_key or node_key in original:
            return node.name
...
def _rejected_edge_record(
    ...
    *,
    ...
    source_name = edge.source_entity_name.strip()
    target_name = edge.target_entity_name.strip()
    return {
        ...
        'reason': reason,
        'miss_type': miss_type,
        'normalized_source': _normalize_string_exact(source_name),
        'normalized_target': _normalize_string_exact(target_name),
...
def _validate_extracted_edges(
    edges: list[Any],
    name_to_node: dict[str, EntityNode | None],
) -> _ValidationResult:
    """Validate edge endpoints against the entity name map.

    Drops edges whose source or target entity name cannot be resolved,
    and self-edges where source and target resolve to the same node.

    Returns:
        _ValidationResult with valid edges, connected entity names, and dropped count.
    """
    valid: list[Any] = []
    ...
    for edge in edges:
        source_name = edge.source_entity_name
        target_name = edge.target_entity_name

        source_node = _resolve_node_name(name_to_node, source_name)
        if source_node is None:
            miss_type = _classify_entity_miss(source_name, name_to_node)
            candidate_source = _candidate_name_for_miss(source_name, name_to_node)
            logger.warning(
                'Source entity not found [%s] edge=%s name=%s normalized=%s candidate=%s',
                miss_type,
                edge.relation_type,
                source_name.strip(),
                _normalize_string_exact(source_name),
                candidate_source or '<none>',
            )
            rejected_edges.append(
                _rejected_edge_record(
                    edge,
                    reason='source_not_found',
                    miss_type=miss_type,
                    candidate_source=candidate_source,
                    fixable=candidate_source is not None,
                )
            )
            dropped += 1
            continue

        target_node = _resolve_node_name(name_to_node, target_name)
        if target_node is None:
            miss_type = _classify_entity_miss(target_name, name_to_node)
            candidate_target = _candidate_name_for_miss(target_name, name_to_node)
            logger.warning(
                'Target entity not found [%s] edge=%s name=%s normalized=%s candidate=%s',
                miss_type,
                edge.relation_type,
                target_name.strip(),
                _normalize_string_exact(target_name),
                candidate_target or '<none>',
            )
            rejected_edges.append(
                _rejected_edge_record(
                    edge,
                    ...
                    miss_type=miss_type,
                    candidate_target=candidate_target,
                    fixable=candidate_target is not None,
                )
            )
            dropped += 1
            continue

        # Normalize names to the actual entity node names
        ...
        if source_node.uuid == target_node.uuid:
            ...
            rejected_edges.append(
                _rejected_edge_record(
                    edge,
                    reason='self_edge',
                    candidate_source=source_node.name,
                    candidate_target=target_node.name,
                    fixable=False,
                )
            )
            dropped += 1
        ...
        valid.append(edge)
        connected.add(source_node.name)
        connected.add(target_node.name)
...
async def extract_edges(
    ...
    second_pass_extraction: bool = False,
    second_pass_mode: str = 'always',
    ...
    # Build name-to-node mapping for validation, including official_name and synonyms.
    name_to_node = _build_name_to_node_map(nodes)
    ...
    # ── First-pass validation (before deciding whether to refine) ──
    validation = _validate_extracted_edges(all_edges_data, name_to_node)
    edges_data = validation.valid_edges

    # ── Post-validation: check rejected edges and entities that lost all edges after validation ──
    if second_pass_extraction:
        node_names = {n.get('name', '') for n in context['nodes'] if n.get('name')}
        validated_disconnected = node_names - validation.connected_names
        fixable_rejected_edges = [edge for edge in validation.rejected_edges if edge.get('fixable')]
        should_refine_after_validation = bool(fixable_rejected_edges) or (
            bool(validated_disconnected)
            and should_refine_edges(
                second_pass_mode, edges_data, context['nodes'], second_pass_min_edges
            )
        )

        if should_refine_after_validation:
            logger.debug(
                'Post-validation edge refinement: %d/%d entities disconnected after validation; '
                '%d fixable rejected edges (first-pass had %d edges, validation dropped %d)',
                len(validated_disconnected),
                len(node_names),
                len(fixable_rejected_edges),
                len(all_edges_data),
                validation.dropped_count,
            )
            refined_edges = await refine_extracted_edges(
                clients,
                episode,
                previous_episodes,
                edges_data,
                ...
                rejected_edges=fixable_rejected_edges,
            )
            # Validate refinement result
            refined_validation = _validate_extracted_edges(refined_edges, name_to_node)
            edges_data = refined_validation.valid_edges

            still_disconnected = node_names - refined_validation.connected_names
            if still_disconnected:
                logger.debug(
                    'After post-validation refinement: %d entities still disconnected: %s',
                    len(still_disconnected),
                    ', '.join(sorted(still_disconnected)[:10]),
                )
