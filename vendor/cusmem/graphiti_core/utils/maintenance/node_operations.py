...
from graphiti_core.utils.maintenance.entity_alignment import unique_preserve_order
...
class _EntityValidationResult:
    """Result of validating extracted entities before node creation."""

    __slots__ = ('valid_entities', 'rejected_entities', 'dropped_count')
    ...
    def __init__(
        ...
        self.rejected_entities = rejected_entities or []
        self.dropped_count = dropped_count


def _entity_type_name_for_id(
    entity_type_id: int,
    ...
    if 0 <= entity_type_id < len(entity_types_context):
        ...
        return str(entity_type_name) if entity_type_name is not None else None
    return None


def _rejected_entity_record(
    ...
    *,
    ...
    return {
        ...
        'official_name': getattr(entity, 'official_name', None),
        'synonyms': list(getattr(entity, 'synonyms', None) or []),
        'episode_indices': list(entity.episode_indices),
        'reason': reason,
        'fixable': fixable,
...
def _validate_extracted_entities(
    ...
    """Validate extracted entities before second-pass refinement or node creation.

    Deterministic rules still decide what cannot be persisted. Fixable records are
    only hints for the second pass; refined output must pass this validation again.
    """
    excluded_types = set(excluded_entity_types or [])
    valid_entities: list[ExtractedEntity] = []
    rejected_entities: list[dict[str, Any]] = []

    for entity in extracted_entities:
        entity_type_name = _entity_type_name_for_id(entity.entity_type_id, entity_types_context)

        if not entity.name.strip():
            rejected_entities.append(
                _rejected_entity_record(
                    entity,
                    reason='empty_name',
                    entity_type_name=entity_type_name,
                    fixable=False,
        ...
        if entity_type_name in excluded_types:
            rejected_entities.append(
                _rejected_entity_record(
                    entity,
    ...
    return _EntityValidationResult(
        valid_entities=valid_entities,
        rejected_entities=rejected_entities,
        dropped_count=len(rejected_entities),
    )
...
async def extract_nodes(
    ...
    validation = _validate_extracted_entities(
        extracted_entities,
        entity_types_context,
        excluded_entity_types,
    )
    filtered_entities = validation.valid_entities

    if second_pass_extraction:
        fixable_rejected_entities = [
            entity for entity in validation.rejected_entities if entity.get('fixable')
        ]
        should_refine_after_validation = bool(fixable_rejected_entities) or should_refine_entities(
            second_pass_mode, filtered_entities, second_pass_min_entities
        )

        if should_refine_after_validation:
            logger.debug(
                'Entity refinement: %d first-pass entities, %d valid, %d rejected, %d fixable',
                len(extracted_entities),
                len(filtered_entities),
                validation.dropped_count,
                len(fixable_rejected_entities),
            )
            refined_entities = await refine_extracted_entities(
                clients,
                episode,
                previous_episodes,
                filtered_entities,
                ...
                rejected_entities=validation.rejected_entities,
            )
            refined_validation = _validate_extracted_entities(
                refined_entities,
                entity_types_context,
                excluded_entity_types,
            )
            filtered_entities = refined_validation.valid_entities
...
def _create_entity_nodes(
    ...
    for extracted_entity in extracted_entities:
        raw_name = extracted_entity.name.strip()
        ...
        # Store model-provided alignment signals for later dedup and edge endpoint checks.
        # Do not rewrite the observed entity name here; OCR and naming variants are
        # handled by official_name/synonyms alignment instead of hardcoded replacements.
        attrs: dict[str, Any] = {}
        official_name = (getattr(extracted_entity, 'official_name', None) or '').strip()
        if official_name and official_name != raw_name:
            attrs['official_name'] = official_name

        synonym_values = unique_preserve_order(
            [str(value) for value in (getattr(extracted_entity, 'synonyms', None) or [])]
        )
        if synonym_values:
            attrs['synonyms'] = synonym_values
...
def _collapse_exact_duplicate_extracted_nodes(
    ...
    """Collapse same-message duplicates with the same (normalized name, type) key.

    Uses type-aware dedup key: same name but different types (e.g. Java+Language
    vs Java+Location) are kept separate. When duplicates agree on key, keeps the
    node with more specific type labels.
    ...
    canonical_by_name: dict[tuple, EntityNode] = {}
    ordered_keys: list[tuple] = []

    for node in extracted_nodes:
        dedup_key = _dedup_key(node.name, node.labels)
        existing = canonical_by_name.get(dedup_key)
        if existing is None:
            canonical_by_name[dedup_key] = node
    ...
    return [canonical_by_name[k] for k in ordered_keys]
