...
def _annotation_allows_type(annotation: Any, expected_type: type) -> bool:
    if annotation is expected_type:
        return True

    origin = get_origin(annotation)
    if origin in (Union, UnionType):
        return any(_annotation_allows_type(arg, expected_type) for arg in get_args(annotation))
...
def _coerce_attribute_value_shape(
    value: Any,
    ...
    if _annotation_allows_type(field_info.annotation, str) and not isinstance(value, str):
        if isinstance(value, dict | list):
            return _to_compact_json(value), True
...
def cap_string_attributes(
    ...
    *,
    ...
    for field_name, value in response.items():
        value, coerced_shape = _coerce_attribute_value_shape(value, model, field_name)
        if coerced_shape:
            logger.info(
                'attribute_shape_coerced prompt=%s group_id=%s entity_uuid=%s field=%s',
                prompt_name,
                group_id or '<unknown>',
