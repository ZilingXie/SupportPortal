...
DEFAULT_SMALL_MODEL = 'deepseek-chat'
...
class BaseOpenAIClient(LLMClient):
    ...
    def _handle_structured_response(self, response: Any) -> tuple[dict[str, Any], int, int]:
        ...
        Supports both OpenAI Responses API (.output_text) and Chat Completions fallback.

        Returns:
            tuple: (parsed_response, input_tokens, output_tokens)
        """
        # Detect response type: Responses API has .output_text, ChatCompletion has .choices
    ...
    def _handle_json_response(self, response: Any) -> tuple[dict[str, Any], int, int]:
        ...
        # Auto-wrap bare lists: DeepSeek sometimes returns bare lists instead of objects
        if isinstance(parsed, list):
            first = parsed[0] if parsed else {}
            if 'summary' in first:
                parsed = {'summaries': parsed}
            elif 'duplicate_candidate_id' in first or 'candidate_id' in first:
                parsed = {'entity_resolutions': parsed}
            elif 'source_entity_name' in first or 'fact' in first:
                parsed = {'edges': parsed}
            elif 'uuid' in first:
                parsed = {'entities': parsed}
            elif 'communityId' in first:
                parsed = {'community_clusters': parsed}
            else:
