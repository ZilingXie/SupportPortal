...
class EntityNode(Node):
    ...
    async def save(self, driver: GraphDriver):
        ...
        if driver.provider == GraphProvider.KUZU:
            ...
            from graphiti_core.helpers import neo4j_safe_attributes

            for k, v in neo4j_safe_attributes(self.attributes or {}).items():
...
class CommunityNode(Node):
    ...
    attributes: dict[str, Any] = Field(default_factory=dict, description='extra properties')
    ...
    async def save(self, driver: GraphDriver):
        ...
        # Save extra attributes as individual Neo4j properties
        from graphiti_core.helpers import neo4j_safe_attributes

        extra = neo4j_safe_attributes(self.attributes or {})
        if extra and driver.provider == GraphProvider.NEO4J:
            for key, value in extra.items():
                await driver.execute_query(
                    'MATCH (n:Community {uuid: $uuid}) SET n[$key] = $value',
                    uuid=self.uuid,
                    key=key,
                    value=value,
                )
...
def get_community_node_from_record(record: Any) -> CommunityNode:
    ...
    # Filter out system fields, keep only extra attributes like topics/key_entities
    extra = {
        k: v
        for k, v in attrs.items()
        if k not in {'uuid', 'name', 'group_id', 'summary', 'created_at', 'name_embedding'}
    }
