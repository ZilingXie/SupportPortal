...
## 2026-09-03 - Production reply validation lowered to a safety floor (p2-140)
...
- Prompt or model version: `automation-persona-v21` → `automation-persona-v22`. Model configuration unchanged.
- Summary: all wording-level blocking regexes are removed from `validate_account_reply_contract`: the fraud/suspension handoff triple (will + contact verb + "24 hours" in one clause), the suspension contact interrogative form ("which/what/preferred … email" within 80 chars), the enablement submission 24h/change-window affirmative clauses, the enablement completion positive enabled/closing/media-relay requirements, the missing-information inline/bullet layout and label-completeness contract, the ownership clause family, and the appid-invalid/not-found sentence requirements. The business points stay as prompt guidance ("Every point named in the reply policy below is required content … in your own words"). The retained blocking safety floor: contract normalization and intent/close derivation, empty response, signature/greeting strip, generation-time forbidden values and engineer source-value bounds, misleading future enable/archive claims (completed/archer), appid-error overclaim bans, the missing-information invented-timeframe ban, and a new suspension affirmative close/archive/reopen claim ban (negation-aware via `_is_positive_clause`, only on the two suspension intents, so "we will not close this ticket" passes). The deterministic missing-information assembly and the enablement-submission contract-sentence append stay unchanged.
...
- Companion behavior changes in the same task: greeting names now resolve message-first via `resolve_customer_greeting_name` (latest customer-comment author → case name → requester → "Customer", per-candidate validity), author name/kind persist into message meta on both stacks; the ECS worker passes the route-pinned `payload.persona` verbatim into all four reply-job exits (no re-resolve); suspension closing assigns the reviewer and then sends a reviewer notification email (Graph) with a persisted `reviewer_notify_email` state and a `zendesk_reviewer_notify_email` event; route_preparation's suspension first draft drops its close/reopen sentence; live-scenario acceptance checks (E1/E2/F1/S1) are decoupled from the production validator (event waits gain state conditions; a scenario-side reply-content checker replaces the validator reuse; S1 no longer asserts `solved`).
...
- Expected behavior change: natural-language replies pass generation validation without retries; a reply that omits a business point (e.g. the 24-hour promise) is no longer auto-rejected to human review — only safety-floor violations are. Suspension replies never claim closing/archiving/reopening affirmatively.
...
- Verification: pending — targeted suites in p2-140.
