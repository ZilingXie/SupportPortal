...
> **历史报告（EC2 split runtime 已于 2026-08-25 退役）：** 本文记录的 EC2 `/automation/staging`、`/automation/preproduction`、`/automation/production` 发布与回滚命令不再用于日常运维。三条 EC2 路径返回 410，ECS 三环境迁移由 `p1-53` 跟踪；现有 EC2 `/production` 保持独立运行。
