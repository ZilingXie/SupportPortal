...
> 这是兼容线上/RDS 数据库调试路径的恢复手册。普通本地开发默认使用 `bash scripts/workflow/restart_single_host_stack.sh --use-local-env`，该路径会按 `.env.local` 的 `local_lightweight` 设置启动轻量栈，但 DB 默认继续使用 `.env` 中的线上/RDS `TICKET_DB_DSN` / `PGVECTOR_DSN`。

适用范围：
1. 当前这台 macOS + Podman + Shadowrocket 的本地 SupportPortal 单机环境。
2. 你使用 `restart_single_host_stack.sh --use-local-env` 复用 `.env` 中的线上/RDS `TICKET_DB_DSN` / `PGVECTOR_DSN`。
...
2. `restart_single_host_stack.sh --use-local-env` 会先自动执行同样的 relay 检查，所以机器重启后通常只需要直接跑这条 remote DB 重启命令。
