...
This user-executed command uses crane to copy the exact OCI manifests and layers accepted in
Preproduction into Production. It never builds an image or changes the Release
Manifest. The target ECR repository must have immutable tags enabled.
EOF
}
...
parse_args() {
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --source-repository) [[ $# -ge 2 ]] || fail "--source-repository requires a value"; SOURCE_REPOSITORY="$2"; shift 2 ;;
...
main() {
  ...
  local role tag expected source_digest target_digest existing
