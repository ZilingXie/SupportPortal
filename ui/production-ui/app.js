...
const PRODUCTION_API_BASE = "/production";
...
function withProductionApiBase(url) {
...
async function accountFetch(url, options = {}) {
  ...
  let response;
  try {
    response = await fetch(withProductionApiBase(url), authRequestInit(requestOptions));
  } catch (error) {
  ...
  return `
    <section class="account-login-page">
      ...
      <footer class="account-login-footer">
        <strong>&copy; 2026 SupportPortal. Secure Account Production Workspace.</strong>
...
async function accountWorkspaceMe(token) {
  const response = await fetch(withProductionApiBase("/api/workspace/me"), {
    cache: "no-store",
...
async function handleAccountLogin(form) {
  ...
  render();
  try {
    const response = await fetch(withProductionApiBase("/api/workspace/auth/login"), {
      method: "POST",
    ...
    <main class="account-shell">
      <aside class="side-panel">
        <div class="brand">
          ...
          <div>
            <div class="eyebrow">Account production</div>
