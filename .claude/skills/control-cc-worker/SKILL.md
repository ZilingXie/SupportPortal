...
For correction payloads, expect `/control-cc-worker` as the first line and `mode: correction` in the payload body, then:
...
Treat `must_change` as the correction objective. Do not ask for `goal` when a correction payload has both `problem` and `must_change`.
